...
错 = []
查过 = 0
# 「……的护身符」这句话是怎么拼出来的:取它所在的那一整条语句。
# 【两个叫法都收】。名字翻过两次（2026-09-01 定「护身符」、
# 2026-09-06 用户拍板改回「御守」），而这一支判的是【拼这句话时有没有问
# 「会不会住进来」】—— 跟叫什么无关，所以两个都收，翻多少次都不用改这里。
# 下面那一版的原话:2026-09-01 屏上统一改叫「护身符」（「御守」是个
# 需要解释的和风外来词，而这一版不要古风）—— 改完之后这一支
# 「查了 0 处拼名」还照样绿:它盯的是旧词，而旧词一个都不剩了。
# 一支查不到东西的门禁比没有门禁更糟，它给的是错的信心。
# 底下那条「一处都没查到就算失效」是同一件事的兜底。
拼 = re.compile(r"[^\n;]{0,200}的(?:御守|护身符)[^\n;]{0,80}")
...
for f in 文件:
    ...
    for i, 行 in enumerate(行们, 1):
        ...
        for m in 拼.finditer(行):
            ...
            if not 守住.search(窗):
                错.append(f'{f.relative_to(根)}:{i}　拼「的护身符」却没判会不会住进来 —— 香也挂着人')

if not 文件:
    print('✗ 一个文件都没扫到 —— 检查器自己失效了'); sys.exit(1)
if 查过 == 0:
    print('✗ 一处「X 的护身符」都没查到 —— 要么这句话被改没了，要么这一支的正则跟不上了。\n'
          '  两种都得有人看一眼:绿着而什么都没查，比红更糟。'); sys.exit(1)
for e in 错:
    print('  ✗ ' + e)
