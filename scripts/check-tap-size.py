...
for 屏, d in m.items():
    for e in d.get('可点', []):
        总 += 1
        # 【0×0 是「不在屏上」，不是「太小」】（2026-09-05）。
        # 弹性槽在矮屏上是收起的（app.wxss:417 `.flexslot{display:none}`），
        # 里面挂着 bindtap 的东西照旧进得了「可点」这一栏，量出来 0×0。
        # 收起的东西点不到，也就谈不上触达面 —— 把它算成「小于 44」
        # 报出来的是一句读不懂的话（`village 0×0 .tally-go`),
        # 而真该修的那几个反倒混在里面。
        #
        # 【但不许默默丢掉】。这个仓的规矩是「有跳过就是没验」，
        # 所以单列一行说清楚:哪几个是收起的、这一轮没算。
        # 头一次撞见它是把点香的窗口挪到当下之后 ——
        # `.tally-go` 只在那二十五分钟里存在，此前从没被量过。
        if e['高'] == 0 or e['宽'] == 0:
            收起的.append(f"{屏}　.{e['类'][:34]}　{e['文'][:14]}")
            continue
        if e['高'] < 底线 or e['宽'] < 底线:
...
if 收起的:
    print(f'  · 这 {len(收起的)} 个是收起的（0×0），这一轮没算：')
    for x in 收起的[:6]:
        print('      ' + x)
