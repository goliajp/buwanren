...
# （定下来的那个, [不许出现的], 为什么是它）
组 = [
    # 【2026-09-06 翻了个面】（用户拍板）。这一行原先钉的是「护身符」,
    # 理由是「中文里现成的词，不用先学一个和风外来词」—— 那个理由没有失效,
    # 但屏上、库里的商品名、验收夹具三处并不一致，
    # 五路体验走查里「新来的」那一路第一眼就撞上了同物异名。
    # 词定了「御守」，而「第一次打开的人不认识它」那件事另有解法:
    # 把村主屏那颗「扫御守」从全屏最抢眼的位置上降下来 ——
    # 它对新来的人最没用（他手上没有御守）。
    # 记在 `.claude/rfcs/20260906-five-walkthroughs.md` 第一条。
    ('御守', ['护身符'], '用户 2026-09-06 拍板;库里的商品名与验收夹具同日跟着改'),
    ('回去',   ['返回'], '三十一处对一处'),
