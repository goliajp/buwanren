...
// 依赖:bun + playwright(chromium)+ docker(造/清测试数据)
// ⚠ 会写数据(两笔订单 + 一个用户),跑完自己清掉。对 dev 库跑。
import { chromium } from 'playwright';
import { execSync } from 'node:child_process';

const psql = (sql) =>
  execSync(`docker exec -i unmei-postgres psql -U unmei -d unmei -tA -c "${sql.replace(/"/g, '\\"')}"`)
    .toString().trim();
...
// ─── 登录 ───
await page.goto('http://localhost:6030/login');
...
// ─── Orders 页 → 打开详情抽屉 ───
await page.goto('http://localhost:6030/orders');
