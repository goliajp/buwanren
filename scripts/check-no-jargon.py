...
行话 = ['大六壬', '梅花易数', '藏历密算', '奇门遁甲', '紫微斗数', '太乙神数',
        ...
        # 【御守】不是术数行话，但屏上同样不许出现，理由是同一个:
        # 它要先学一个词才读得懂。这是个和风外来词，第一次打开的人
        # 不知道要扫什么（第一轮评审「第一次打开的人」那一路点过），
        # 而村主屏的按钮早就改叫「扫护身符」了 —— 于是同一个东西
        # 在同一屏上有两个名字（2026-09-01 第二轮评审）。
        # 全部统一成「护身符」:中文里现成的词，不用学。
        # 代码里的 `omamori`（分类名、表名、接口路径）不受这条约束。
        '御守']
