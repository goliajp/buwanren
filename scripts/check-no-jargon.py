...
# 【「御守」2026-09-06 从上面这张表里拿掉了】——用户拍板统一用这个词。
#
# 它原先在表里，理由记在这儿，因为那个理由**没有失效**:
# 「御守」是和风外来词，第一次打开的人不知道要扫什么、扫哪儿、
# 自己手上有没有（第一轮评审「第一次打开的人」那一路点过）。
# 2026-09-01 的处置是把村主屏那颗按钮改叫「扫护身符」,
# 而那反倒造出了更糟的第三种状态:**同一样东西两个名字**
# —— 五路体验走查里「新来的」那一路第一眼就撞上了。
#
# 现在的分工是这样：
#   · 名字统一到「御守」（屏上、库里的商品名、验收夹具都改了）
#   · 这一支只是不再禁这个词。**同物异名不归它管** ——
#     `scripts/check-one-name.py` 早就在守这件事（那一支的头一行
#     记的就是这一对），2026-09-06 把它钉的那个词翻成了「御守」。
#     两处各写一份判据必然走散，所以这里不留副本。
#   · 「第一次打开的人不认识这个词」那件事，真正的解法不是换个说法,
#     是**把那颗按钮从第一屏最抢眼的位置上降下来**:
#     它对新来的人最没用（他手上没有御守），却是全屏权重最高的东西。
#     记在 `.claude/rfcs/20260906-five-walkthroughs.md` 第一条。
#
# 代码里的 `omamori`（分类名、表名、接口路径）本来就不受这条约束。
