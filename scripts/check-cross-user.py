...
if bad:
    ...
    sys.exit(1)
# 【探到的太少就不算数】（2026-09-04）。
# 上面每一条 `if` 都会在造不出对象时静静跳过 ——
# 订单那三条一起没了的话，这一支只探了本命那四条，
# 而钱那一整片一次都没被碰过，报的却仍然是「都碰不到」。
# 这一支修的是同一个形状:探不到跟一条都没漏长得一模一样。
if len(probes) < 6:
    print(f'✗ 只探了 {len(probes)} 处（该有七处）—— 有整片没探到，'
          '这时候的「都碰不到」不算数')
    sys.exit(1)
print(f'✓ {len(probes)} 处都碰不到')
