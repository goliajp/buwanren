...
【第二件事:内容排到栏底下去】（2026-09-06 自己撞的）。
上面那一半只看「固定层之间」。而更常见的一种是**内容**排到固定层底下 ——
屏底留白（`.page` 的 `padding-bottom`）是对着那一栏的实测高度写死的，
而那一栏会随内容长高。给确认屏加一行协议链接，栏从 90px 长到 134px，
留白还是 98px:最后一行明细整个落在成交栏底下，屏上看不见。

**当时没有一支门禁看得见它**：一屏放得下那一支只比总高（`min-height: 100vh`
让它正好等于视口）、这一支只比固定层之间、截屏是一张图。
数据一直都在（`主块` 与 `钉住的` 两栏都是实测外接矩形），只是没人比过。

数据来自镜像 `web/shots.mjs` 的 `钉住的` 与 `主块` 两栏 —— 真实排版下的外接矩形。
"""
import json
import os
import pathlib
...
# ── 内容有没有排到钉住的那一块底下 ────────────────────────────
# 判据:`主块` 里【不是那一块本身、也不在它里面】的元素，
# 底边不许越过那一块的顶边。容差 2px 给亚像素舍入。
内容压着 = 0
for 屏, d in m.items():
    for 栏 in d['钉住的']:
        # 只管钉在屏底那种（顶边在视口下半）—— 钉在顶上的那种（导航条）
        # 内容本来就该从它下面开始，不是同一件事
        视高 = (d.get('视口') or {}).get('高') or 667
        if 栏['顶'] < 视高 / 2:
            continue
        for 块 in d.get('主块', []):
            if 块['类'] == 栏['类']:
                continue                      # 就是它自己
            if 块['顶'] >= 栏['顶'] - 2:
                continue                      # 整个在栏里或栏下面（多半是它的子块）
            底 = 块['顶'] + 块['高']
            if 底 > 栏['顶'] + 2:
                内容压着 += 1
                错.append(f'{屏}　内容 .{str(块["类"])[:20]}（排到 {底}）'
                          f'落在钉住的 .{str(栏["类"])[:20]} 底下（栏顶 {栏["顶"]}）'
                          f'—— 屏上看不见这 {底 - 栏["顶"]}px　「{块["文"][:12]}」')
...
for e in 错:
    print('  ✗ ' + e)
print(('✗ ' if 错 else '✓ ')
      + f'钉住的那几块没互相压、内容也没排到它们底下 · {len(m)} 屏 · {块数} 块')
