...
# 而不是假绿 —— 那正是它该有的样子。
# 2026-08-30:0830 版把这一句从「一事一签 · 想清楚再摇 —— 点中心，或摇手机」
# 改成了「想清楚一件事，点中间，或者摇一摇」。锚点跟着换。
# 2026-08-31:又改了一次（「想清楚一件事」被读成「把这件事想清楚」，
# 而这里要的是「心里装着一件事」）。**同一处锚点被文案打断三回了** ——
# 所以这次不再钉具体的字，钉那一行的【类名】:`compass-hint` 是结构，
# 文案怎么打磨都不动它。变异往那一行的行尾塞一个半角逗号。
#
# 每次它报的都是「变异没植进去（基准源码变了？）」而不是假绿 ——
# 那正是它该有的样子:锚点没了就说没了，不假装绿。
mutate "界面上多了个半角逗号" check-punct-ui \
  "edit('mini/miniprogram/pages/home/index.wxml', '或者摇一摇</view>', '或者摇一摇,</view>')"
