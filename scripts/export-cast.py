...
# `--check` 只比对不写（给门禁用）。位置参数照旧，但要把它挑出去，
# 否则 `--check` 会被当成「源文件路径」—— 第一版就是这么报的 FileNotFoundError。
ARGS = [a for a in sys.argv[1:] if not a.startswith('--')]
CHECK = '--check' in sys.argv

SRC = Path(ARGS[0]) if len(ARGS) > 0 else ROOT / 'rooms' / 'design.html'
OUT = Path(ARGS[1]) if len(ARGS) > 1 else ROOT / 'backend' / 'seed' / 'villagers.sql'
...
ON CONFLICT (id) DO UPDATE SET
  ...
  voice = EXCLUDED.voice, rarity = EXCLUDED.rarity;''')
    ...
    if CHECK:
        # 只比对，不写。给门禁用：设计册改了却没重导，库里的字就是旧的，
        # 而那件事没有任何地方看得出来 —— 房间那边早有同样的护栏
        # （`rooms/tools/build.js --check`），村民表这边一直没有。
        old = OUT.read_text(encoding='utf-8') if OUT.exists() else ''
        if old != body:
            print(f'✗ {OUT.relative_to(ROOT)} 与设计册对不上 —— 改了 design.html 就要重导：')
            print('    python3 scripts/export-cast.py')
            sys.exit(1)
        print(f'✓ {OUT.relative_to(ROOT)} 与设计册一致（村民 {len(cast)} 位 · 术数 {len(arts)} 门）')
        return
