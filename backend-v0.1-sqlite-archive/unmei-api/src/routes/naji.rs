...
async fn spin(
    ...
    let user_row = sqlx::query!(
        "SELECT active_natal_id, platform, region, locale FROM user WHERE id=?", c.sub
    ).fetch_one(&st.db).await?;

    let (primary_yongshen, avoid_wuxing) = if let Some(nid) = user_row.active_natal_id.as_ref() {
        let s = sqlx::query!(
            "SELECT primary_yongshen, avoid_wuxing FROM natal_summary WHERE natal_id=?",
            nid
        ).fetch_optional(&st.db).await?;
        if let Some(s) = s {
            let av: Vec<String> = serde_json::from_str(&s.avoid_wuxing).unwrap_or_default();
    ...
    let suit_json = serde_json::to_string(&yi)?;
    let avoid_json = serde_json::to_string(&ji)?;
    let t_chart_str = if t_chart_json.is_null() { None } else { Some(t_chart_json.to_string()) };
    let rec_id = rec.as_ref().map(|r| r.id.clone());
    let signed_seed = seed as i64;
    let nid_for_log = user_row.active_natal_id.clone();
    let yr = year; let mo = month as i64; let d = day as i64;
    let h = hour as i64; let mi = minute as i64;
    sqlx::query!(
        r#"INSERT INTO naji_record
           (id, user_id, natal_id, asked_year, asked_month, asked_day, asked_hour, asked_minute, asked_tz,
            ...
            suit_words, avoid_words, quote_id, recommended_product_id, seed)
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"#,
        id, c.sub, nid_for_log, yr, mo, d, h, mi, tz_offset,
        req.location_lat, req.location_lon, t_chart_str,
...
async fn history(
    ...
    let rows = sqlx::query!(
        r#"SELECT id, asked_at, gate, direction, suit_words, avoid_words,
                  asked_year, asked_month, asked_day, asked_hour
           FROM naji_record WHERE user_id=?
           ORDER BY asked_at DESC LIMIT 50"#,
        c.sub
    ).fetch_all(&st.db).await?;
    let mut v = Vec::with_capacity(rows.len());
    for r in rows {
        let date = format!("{:02}·{:02} {}时",
...
async fn detail(
    ...
    let r = sqlx::query!(
        r#"SELECT id, asked_at, gate, direction, gate_explain, suit_words, avoid_words, quote_id, recommended_product_id
           FROM naji_record WHERE id=? AND user_id=?"#,
        id, c.sub
    ).fetch_optional(&st.db).await?;
    let r = r.ok_or_else(|| ApiError(unmei_domain::AppError::NotFound("naji".into())))?;
    let yi: Vec<String> = serde_json::from_str(&r.suit_words).unwrap_or_default();
    let ji: Vec<String> = serde_json::from_str(&r.avoid_words).unwrap_or_default();
    let q = if let Some(qid) = r.quote_id {
        let qr = sqlx::query!("SELECT book, chapter, text FROM quote WHERE id=?", qid)
            .fetch_optional(&st.db).await?;
...
async fn check_badges(st: &AppState, user_id: &str) -> Result<(), ApiError> {
    ...
    let count_row = sqlx::query!("SELECT COUNT(*) as c FROM naji_record WHERE user_id=?", user_id)
        .fetch_one(&st.db).await?;
    let count = count_row.c;
    let badges = sqlx::query!("SELECT id, code, rule_dsl FROM badge WHERE status='active'")
        .fetch_all(&st.db).await?;
    for b in badges {
        let rule: serde_json::Value = serde_json::from_str(&b.rule_dsl).unwrap_or(serde_json::Value::Null);
        ...
        if typ == "count" && action == "naji.spin" && count >= threshold {
            ...
            let exists = sqlx::query!(
                "SELECT badge_id FROM user_badge WHERE user_id=? AND badge_id=?",
                user_id, b.id
            ).fetch_optional(&st.db).await?;
            if exists.is_none() {
                sqlx::query!("INSERT INTO user_badge (user_id, badge_id) VALUES (?,?)", user_id, b.id)
