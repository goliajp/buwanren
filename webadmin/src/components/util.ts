 ...
 *
 * 【从 Users 页提上来的】（2026-09-04 · 25 计划的后台逐页走）。
 * 它原先是那一页的私有函数，注释就写着上面那句 —— 而主数据页
 * 「可见 region」那一列渲的正是裸的 `cn` / `verify`,
 * 同一句道理只在一处生效。
 *
 * `verify` 与 `p25` 是这个仓库自己造的区:一个给
 * `verify-semantics.sh` 的校验商品，一个给 25 计划的验收商品 ——
...
export function regionLabel(r?: string | null): string {
  return { cn: '中国大陆', hk: '香港', tw: '台湾', jp: '日本',
           ...
           /* 【`zz` 不是一个区】（2026-09-04 · 枚举那一支门禁刚接上就抓到的）。
              ...
              代码里一处都写不出它 —— ISO 3166 里 `zz` 就是
              「未知 / 未指定」，多半是当时某条链路没拿到区就落了库。
              屏上说「没记下来」而不是「zz」:后者读起来像一个真的地方。 */
           zz: '没记下来' }[r ?? ''] ?? (r ?? '—');
}
