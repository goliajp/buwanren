...
/* 启动参数。
   ...
   --allow-file-access-from-files
     村子的贴图是 file:// 来的一张真 PNG。不加这条,凡是画过它的画布都算
     「被跨源污染」,toDataURL / getImageData 直接抛 —— 这是村子当年进不来的直接原因。

   --disable-gpu
     **GPU 光栅化本身不确定**。实测:村子静态层的绘制指令序列每次完全相同
     (给那两个伪随机加计数器验过,烘焙前 0 次、烘焙后 6412 / 3 次,次次一样),
     出来的像素却在几个值之间跳;关掉 GPU 后四次加载哈希完全一致。
     所以飘的不是代码是后端。视觉回归比的是哈希,后端飘一点这道门就废了。 */
...
const SHOOT = () => {
  ...
  /* 村子。那三千多行里绝大部分是地形与房屋,烘在静态层上、与时间无关,钉得住。
     村民的走位与对话是有状态的、没有复位入口,快照它们量到的会是「页面活了多久」——
     这支工具在房间上已经栽过一次,不再栽第二次,所以只钉两张烘好的层。 */
  const L = window.VILLAGE_LAYERS
  if (L && L.bg && L.fg) {
    out.VILLAGE = {}
    for (const k of ['bg', 'fg']) out.VILLAGE[k] = L[k].toDataURL('image/png')
  }
  return out
}
