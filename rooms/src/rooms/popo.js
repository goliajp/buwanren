...
(function () {
  ...
  globalThis.POPO_ROOM = {
    ...
    perform: {
      ...
      stateKey: 'casting',                      // 牌、水晶球、五芒星阵都读这个键
      lineGap: 7200,
      /* 【说的和做的得是同一件事】（2026-09-01 第二轮评审 · 文案）。
         这九句原先说的全是水晶球:「雾散得慢」「这团雾」「球里的人在笑呢」——
         而按钮写着「翻张牌」，她的卡片、名册、屋子副标上都是「翻牌」。
         四个信号里三个说牌，玩家读到的是一屋子对不上的话。
         改成说牌。第八、九句留着不动 —— 那两句是她这个人本身:
         替所有人翻了一辈子，自己那副压在箱底（她缺的是亲人）。
         第二句原先是「我不给你下结论的」，那是全 app 复读了七遍的一句;
         换成她自己的说法:牌上有什么念什么，没有的不添。 */
      lines: ['坐近些呀。手别抖，牌跟着你抖的',
              '先洗一遍。心里想着那件事，别说出来',
              '我只念牌上有的。牌上没有的，婆婆不添',
              '这张是正的。正的就先松口气吧',
              '哎哟，别盯着牌面看。看我，我讲给你听',
  ...
  globalThis.POPO_POSES = POSES
  globalThis.POPO_PALETTE = PC
...
.KK....KK.`)
  ...
  const ACTS = [
    /* x 从 652 挪到 590：水晶球在 666，跟她原来的位置几乎正对，
       ...
       【挪到桌子【左边】站着】（2026-09-03 角色放大之后）。
       先试了往下挪（y 964 → 1120），结果更糟:38%。
       染色重渲一看，她整个人落进了读牌桌的桌肚里 —— 桌子在她【前面】。
       这一屋的家具是按 y 排序绘制的，往下挪等于往桌子后面钻。
       正解是往左让开那张桌（x 590 → 420），她仍然守着牌，人整个露出来。

       【原先的记录 · y 从 964 挪到 1120】（2026-09-03 角色放大之后）。
       所有活物的 scale 从 8 提到 11（主人原先在屏上只有 27×31，
       比同屋家具的中位数还小）—— 放大之后她的上半身正好被
       读牌桌（高 250）的桌面切掉，`see-host` 当场从 84% 掉到 47%。
       挪到桌前:牌还在她手边，人整个露出来。
       这条门禁量的是「露出多少」，而它这次真的挡住了一次退步。 */
    { id: 'read',   node: 'C1', x: 420, y: 1120,  poses: ['gaze'],           fps: 1,   dur: [8, 12], flip: false, w: 4,
      /* 中间那句原先是「我不给你下结论的。我只说我看见什么」——
         跟她表演态那九句里的一句撞了（那一句已改成「我只念牌上有的」），
         同一间屋里同一个意思说两遍。换成只有她会说的一句。 */
      say: ['牌是这么说的哦。信不信在你呀', '这一副我翻了几十年，还是喜欢它', '这张……嗯，不算坏事'] },
  ...
  function startWalk(a) {
    ...
    if (a.flyMode) {   // 飞:不寻路,直接起飞
      ...
      st.until = globalThis.ENGINE_HOST.now() + (a.dur[0] + Math.random() * (a.dur[1] - a.dur[0])) * 1000
      return
    }
    globalThis.startWalkTo(st, a, globalThis.POPO_ROOM, 'popo')
  }
  ...
  const cst = { mode: 'act', act: CATS[0], x: 508, y: 1740, tx: 508, ty: 1740,
                until: globalThis.ENGINE_HOST.now() + 12000, frame: 0, path: [] }
  function catWalk(a) { globalThis.startWalkTo(cst, a, globalThis.POPO_ROOM, 'popocat', { foot: [0, 0] }) }

  // ── 猫头鹰:栖架上不动,只眨眼 ────────────────────────────────
  const owl = { blinkUntil: 0, nextBlink: globalThis.ENGINE_HOST.now() + 4000 }
  ...
  ].map(p => Object.assign(p, { tx: p.x, ty: p.y, mode: 'act', until: globalThis.ENGINE_HOST.now() + 8000, i: 0 }))
  ...
  const portal = { state: 'idle', until: globalThis.ENGINE_HOST.now() + 6000, kind: null, x: 1280, y: 1960, sayUntil: 0 }
  ...
  function frame(mainG, t, cv) {
    ...
    // ── 表演态:玩家点「请婆婆翻张牌」────────────────────────
    if (!POPO_PERF) {
      POPO_PERF = globalThis.wirePerform(globalThis.POPO_ROOM)
    ...
    globalThis.stepPerform(globalThis.POPO_ROOM, st, t, { startWalk, pick })
    ...
    } else if (st.act.fly ? globalThis.stepFly(st, st.act.speed || 30)
                          : globalThis.stepWalk(st, st.act.speed || 7)) {     // 走得慢 —— 七十几岁了;骑上扫帚就不慢了
      st.mode = 'act'
    ...
    if (cst.mode === 'act') {
      if (t > cst.until) {
        const a = globalThis.pickAct(CATS, cst.act)
        if (a) catWalk(a)
      }
    } else if (globalThis.stepWalk(cst, 3)) {
      cst.mode = 'act'
    ...
    for (const p of PETS) {
      if (p.mode === 'act') {
        if (t > p.until) {
          ...
          const pt = p.pts[p.i]
          globalThis.startWalkTo(p, { x: pt[0], y: pt[1] }, globalThis.POPO_ROOM, 'popopet', { foot: [0, 0] })
        }
      } else if (globalThis.stepWalk(p, p.sp)) {
    ...
    if (st.mode === 'walk') {
      const wp = globalThis.walkPose(st, { rate: 2 })
    ...
    pose = globalThis.idlePose(st, pose, 'popo')
    // 婆婆的行动方式是骑扫帚 —— 移动中(飞行)与翻牌(悬空)时离地,
    ...
    const ents = []
    for (const p of PETS) {
      const ps2 = globalThis.actorSprite('popopet', p.pose, false)
      ents.push(globalThis.placeActor('popopet', p.x + ps2.width / 2, p.y + ps2.height,
                                  p.pose, (p.tx - p.x) < 0, {}))
    }
    const opose = (t < owl.blinkUntil) ? 'owl2' : 'owl1'
    const os = globalThis.actorSprite('popoowl', opose, false)
    ents.push(globalThis.placeActor('popoowl', 200 + os.width / 2, 1604 + os.height, opose, false, {}))
    if (portal.state === 'out' && portal.kind) {
      const ks = globalThis.actorSprite('popopet', portal.kind.pose, false)
      ents.push(globalThis.placeActor('popopet', portal.x + ks.width / 2, portal.y + ks.height,
                                  portal.kind.pose, false, {}))
    }
    const cs = globalThis.actorSprite('popocat', cpose, cflip)
    ents.push(globalThis.placeActor('popocat', cst.x + cs.width / 2, cst.y + cs.height, cpose, cflip, {}))
    ents.push(globalThis.placeActor('popo', drawX + globalThis.ACTORS.popo.foot[0],
                                drawY + globalThis.ACTORS.popo.foot[1], pose, flip, popoAtt))

    globalThis.renderRoom(mainG, globalThis.POPO_ROOM, t, ents)
    ...
    if (!cv.__interaction && globalThis.attachRoomInteraction)
      globalThis.attachRoomInteraction(cv, globalThis.POPO_ROOM, {
        onClick(id) {
          if (id !== 'popo_drawer') return
          // 状态放在 room 上(引擎 roomState)，素材的 variant 自己读它。
          globalThis.roomState(globalThis.POPO_ROOM, { drawerOpen: !((globalThis.POPO_ROOM.state || {}).drawerOpen) })
        }
      })
    if (globalThis.drawInteraction) globalThis.drawInteraction(mainG, globalThis.POPO_ROOM, cv, t)

    // 角色附属绘制全部走引擎:房间只声明「谁 · 在哪 · 什么情绪 · 说什么」
    if (cst.mode === 'act' && cst.act.zzz)
      globalThis.drawEmote(mainG, 'zzz', globalThis.actorAnchor('popocat', cpose, cst.x, cst.y), t)
    if (portal.state === 'out' && portal.kind && t < portal.sayUntil)
      globalThis.drawSay(mainG, globalThis.actorAnchor('popopet', portal.kind.pose, portal.x, portal.y),
                     portal.kind.say, { ink: '#1a1620', paper: '#f6efdc' })

    const anc = globalThis.actorAnchor('popo', pose, drawX, drawY)
    if (st.mode === 'act') {
      if (st.act.zzz) globalThis.drawEmote(mainG, 'zzz', anc, t)
      if (st.act.id === 'feed' || st.act.id === 'knit') {
        if ((st.frame >> 4) % 2) globalThis.drawEmote(mainG, 'heart', anc, t)
      }
    }
    // 婆婆的台词纸=淡紫（魔法小屋），与她屋里的紫灰石墙同一支色系
    if (st.sayText && t < st.sayUntil) globalThis.drawSay(mainG, anc, st.sayText, { ink: '#2e2440', paper: '#f2ecfa' })
    else if (t >= st.sayUntil) st.sayText = null
  }
  globalThis.POPO_FRAME = frame
