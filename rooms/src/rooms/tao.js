...
(function () {
  ...
  globalThis.TAO_POSES = POSES
  globalThis.TAO_PALETTE = PC
  pdef('stand', `
...KK....KK.
..KHHK..KHHK
..KHHHKKHHHK
...
..KKLLKKLLKK..`)
  ...
  const af = i => globalThis.ACTORS.tao.foot[i]
  ...
  const st = {
    ...
    until: globalThis.ENGINE_HOST.now() + 5000, frame: 0, tx: 0, ty: 0,
  ...
  let TAO_PERF = null
  function startWalk(a) { globalThis.startWalkTo(st, a, globalThis.TAO_ROOM, 'tao') }
  ...
  const rst = { mode: 'act', act: RACTS[0], x: 1160, y: 1500, until: globalThis.ENGINE_HOST.now() + 4000, tx: 0, ty: 0 }
  ...
  const dog = {
    ...
    frame: 0, until: globalThis.ENGINE_HOST.now() + 4000, ball: null, chases: 0, say: '', sayUntil: 0,
  }
  ...
  function pickR() { return globalThis.pickAct(RACTS, rst.act) }
  ...
  function frame(mainG, t, cv) {
    ...
    if (rst.mode === 'act') {
      ...
      if (t > rst.until) globalThis.startWalkTo(rst, pickR(), globalThis.TAO_ROOM, 'rabbit', { foot: [0, 0] })
    } else if (globalThis.stepWalk(rst, 4)) {
      {
        rst.mode = 'act'
    ...
    if (dog.state === 'roam') {
      ...
      const res = globalThis.stepPursuit(dog, dog.spec || {}, world, t,
        { x: dclampX, y: dclampY })
    ...
    globalThis.roomState(globalThis.TAO_ROOM, {
    ...
    // ── 表演态:玩家点「请桃桃起一局」──────────────────────────
    if (!TAO_PERF) {
      TAO_PERF = globalThis.wirePerform(globalThis.TAO_ROOM)
      ...
      globalThis.TAO_ROOM.acts = [{ actor: 'tao', list: ACTS }, { actor: 'rabbit', list: RACTS, anchorIsFoot: true }]
    }
    globalThis.stepPerform(globalThis.TAO_ROOM, st, t, { startWalk, pick })

    // ── 桃桃:与家具同为可排序实体 ──────────────────────────────
    // 姿态属于行为系统,先算;绘制交给引擎,她因此与柜/床/秋千一起 y-sort。
    let pose, flip = false, drawX = st.x, drawY = st.y
    if (st.mode === 'walk') {
      const wp = globalThis.walkPose(st, { rate: 2 })   // 走位姿态归引擎；rate 是本角色的步频
      ...
      if (st.act.sway)   // 坐在秋千上:摆动量问秋千要,房间不另算一份
        drawX = st.x + globalThis.ASSETS.tao_swing.swayAt(globalThis.TAO_ROOM.state || {}, t)
    }
    ...
    pose = globalThis.idlePose(st, pose, 'tao')
    ...
    const ents = []
    const rs = globalThis.actorSprite('rabbit', rpose, false)
    ents.push(globalThis.placeActor('rabbit', rst.x + rs.width / 2, rst.y + rs.height,
                                rpose, (rst.tx - rst.x) < 0, {}))
    if (dog.ball) {
      const bb = Math.abs(Math.sin(t / 180)) * 6
      ents.push(globalThis.placeProp('tao_ball', dog.ball.x + 20, dog.ball.y - bb, {}))
    }
    const ds = globalThis.actorSprite('dog', dpose, dflip)
    ents.push(globalThis.placeActor('dog', dog.x + ds.width / 2, dog.y + dbob + ds.height,
                                dpose, dflip, {}))
    ents.push(globalThis.placeActor('tao', drawX + af(0), drawY + af(1), pose, flip, taoAtt))
    globalThis.renderRoom(mainG, globalThis.TAO_ROOM, t, ents)
    // L8 UI:可点道具的高亮与叙事气泡。**交互是角色档案的出口** ——
    // 这一句此前【完全没接】,于是四十多件素材声明了 clickable/say 却一件都点不到。
    // 「素材有属性」和「交互接上了」是两件事,PLAYBOOK §4.10 的老账又犯一次。
    if (!cv.__interaction && globalThis.attachRoomInteraction) globalThis.attachRoomInteraction(cv, globalThis.TAO_ROOM, {})
    if (globalThis.drawInteraction) globalThis.drawInteraction(mainG, globalThis.TAO_ROOM, cv, t)



    if (rst.mode === 'act' && rst.act.zzz)
      globalThis.drawEmote(mainG, 'zzz', globalThis.actorAnchor('rabbit', rpose, rst.x, rst.y), t)
    if (dog.sayUntil > t)
      globalThis.drawSay(mainG, globalThis.actorAnchor('dog', dpose, dog.x, dog.y), dog.say || '汪',
                     { ink: '#3a2c20', paper: '#f6efdc' })


    // 角色附属绘制全部走引擎:房间只声明「谁 · 在哪 · 什么情绪 · 说什么」。
    // 锚点由 sprite 自己算,不写 st.x + 40 这类换算 —— 姿态宽度一变就会错。
    const anc = globalThis.actorAnchor('tao', pose, drawX, drawY)
    if (st.mode === 'act') {
      if (st.act.zzz) globalThis.drawEmote(mainG, 'zzz', anc, t)
      if (st.act.heart && ((st.frame >> 4) % 2)) globalThis.drawEmote(mainG, 'heart', anc, t)
      if (st.act.circle) globalThis.drawEmote(mainG, 'sulk', anc, t)
    }
      ...
      if (st.act && st.act.id === 'live' && st.mode === 'act' && Array.isArray(st.act.say)) {
        ...
        globalThis.drawSay(mainG, anc, st.act.say[Math.floor(t / 4500) % st.act.say.length], TAO_SAY)
      } else if (st.sayText && t < st.sayUntil) globalThis.drawSay(mainG, anc, st.sayText, TAO_SAY)
      else if (t >= st.sayUntil) st.sayText = null
  }
  globalThis.TAO_FRAME = frame
