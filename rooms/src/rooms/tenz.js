...
..KFK..KFK..
...KK...KK..`)
  globalThis.TENZ_POSES = POSES
  globalThis.TENZ_PALETTE = PC

  globalThis.TENZ_ROOM = {
    ...
    perform: {
      ...
      lineGap: 5000,
      lines: ['我翻翻日子……', '这天的火跟土是搭的，好日子',
              '初八别开工，改初十', '这个方位，宜。放心去',
              '嘿哈——交给我', '算日子我最准'],
    },
  }
  ...
  function startWalk(a) { globalThis.startWalkTo(st, a, globalThis.TENZ_ROOM, 'tenz') }
  let lastT = 0
  ...
  function frame(mainG, t, cv) {
    ...
    if (!TENZ_PERF) {
      TENZ_PERF = globalThis.wirePerform(globalThis.TENZ_ROOM)
      ...
      globalThis.TENZ_ROOM.acts = [{ actor: 'tenz', list: ACTS }]
    }
    globalThis.stepPerform(globalThis.TENZ_ROOM, st, t, { startWalk, pick })
    // 打木人时桩身转 —— 状态在【真的站定开打】那一刻置起,不是走过去的路上
    globalThis.roomState(globalThis.TENZ_ROOM, { dummy: st.mode === 'act' && !!st.act.dummy })

    if (st.mode === 'act') {
      if (t > st.until) startWalk(pick())
    } else if (globalThis.stepWalk(st, st.act.speed || 11)) {
      st.mode = 'act'
    ...
    pose = globalThis.idlePose(st, pose, 'tenz')
    ...
    let footX = drawX + globalThis.ACTORS.tenz.foot[0], footY = drawY + globalThis.ACTORS.tenz.foot[1]
    let opt = att
    if (st.mode === 'act' && st.act.hops) {
      const h = globalThis.stepHop(st, st.act.hops, t, { period: 760, height: 52 })
      footX = h.x; footY = h.y
    ...
    const ents = [globalThis.placeActor('tenz', footX, footY, pose, flip, opt)]
    globalThis.renderRoom(mainG, globalThis.TENZ_ROOM, t, ents)

    if (!cv.__interaction && globalThis.attachRoomInteraction) globalThis.attachRoomInteraction(cv, globalThis.TENZ_ROOM, {})
    if (globalThis.drawInteraction) globalThis.drawInteraction(mainG, globalThis.TENZ_ROOM, cv, t)

    const anc = globalThis.actorAnchor('tenz', pose, drawX, drawY)
    if (st.act.zzz && st.mode === 'act') globalThis.drawEmote(mainG, 'zzz', anc, t)
    if (st.act.notes && st.mode === 'act') globalThis.drawEmote(mainG, 'note', anc, t)
    if (st.sayText && t < st.sayUntil) globalThis.drawSay(mainG, anc, st.sayText, { ink: '#3a2c20', paper: '#f0e4d0' })
    else if (t >= st.sayUntil) st.sayText = null
  }
  globalThis.TENZ_FRAME = frame
