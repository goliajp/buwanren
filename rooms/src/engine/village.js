...
....KnK...`)
  ...
  const RULES = [
    ...
    /* 【句式单调这条量过了，不改】（2026-09-02）。
       第二轮评审说「160 句里 113 句是同一个两拍句式」。实测:
       单句 bark 共 255 条，恰好一个逗号的两拍句 104 条 = 40%;
       另有 105 条整句不带标点。按角色分:阿云 55%、婆婆 45%、
       桃桃 44%、丹增 40%、路人 17%。
       对一句话的中文口语来说，「A，B」本来就是最常见的形状，
       四成落在这个区间是自然的，不是模板。真要担心的是某个角色
       一个句式说到底 —— 最高的阿云 55%，离那还远。
       这条不动。留着这段数，免得下一轮再议一遍。 */
  ...
  const CONVOS = [
    ...
    { a:'tao', b:'ayun', p:'market', cd:240, L:[['a','早市的桃子被我包了'],['b','贫道只想买两个'],['a','……给你一个'],['b','……谢了']] },
