  ...
  globalThis.SAY_STYLE = { ink: '#3a2c20', paper: '#fdf2f5' }

  // 锚点:sprite 顶边中心。房间不再自己写 st.x + 40 这类换算 ——
  // 换算写死过一次,姿态一换宽度就对不上。
  globalThis.actorAnchor = function (id, pose, x, y, key) {
    const a = globalThis.ACTORS[id]
    if (key && a && a.anchors && a.anchors[key])
      return { x: x + a.anchors[key][0], y: y + a.anchors[key][1] }
    const cv = globalThis.actorSprite(id, pose, false)
    return { x: x + cv.width / 2, y: y }
  }

  globalThis.drawSay = function (g, anchor, text, style) {
    if (!text) return
    style = style || globalThis.SAY_STYLE
    g.save()
    g.font = SAY.font
    // 折行:中文逐字断,西文遇空格断
    const lines = []
  ...
  const EMOTES = {}
  globalThis.defineEmote = function (kind, fn) { EMOTES[kind] = fn }
  globalThis.drawEmote = function (g, kind, anchor, t) {
  ...
  globalThis.roomAnchorOf = function (room, assetId, key) {
    const a = globalThis.ASSETS[assetId]; if (!a || !room || !room.plan) return null
  ...
  globalThis.drawLink = function (g, from, to, opt) {
    opt = opt || {}
