  ...
  globalThis.invalidateRoomBake = function (room) { BAKE0.delete(room) }
  ...
  globalThis.roomState = function (room, patch) {
  ...
  globalThis.resetRoomState = function (room) {
  ...
  globalThis.roomGrid = function (room, force) {
  ...
  globalThis.gridPath = function (room, sx, sy, tx, ty) {
    const { G, COLS, ROWS, CELL } = globalThis.roomGrid(room)
  ...
  globalThis.faceOf = function (dx, dy) {
  ...
  globalThis.idlePose = function (self, pose, actorId) {
  ...
  globalThis.stepPace = function (self, range, speed) {
  ...
  globalThis.walkPose = function (self, opt) {
    opt = opt || {}
    const rate = opt.rate || 2                    // 帧移位:数字越大越慢
    const f = (self.frame >> rate)
    const d = globalThis.faceOf(self.tx - self.x, self.ty - self.y)
    if (d.face === 'side')
  ...
  globalThis.pickAct = function (acts, current, opt) {
    opt = opt || {}
  ...
  globalThis.startWalkTo = function (self, act, room, actorId, opt) {
    self.mode = 'walk'; self.act = act
    // 锚点语义有两种:主角的 act 坐标是 sprite 左上角(要加 foot 换成脚点)，
    // 宠物的 act 坐标【本身就是脚点】。后者传 { foot: [0,0] }，不要再偏移一次。
    const F = (opt && opt.foot) || (globalThis.ACTORS[actorId] && globalThis.ACTORS[actorId].foot) || [0, 0]
    const pts = globalThis.gridPath(room, self.x + F[0], self.y + F[1], act.x + F[0], act.y + F[1])
  ...
  globalThis.stepWalk = function (self, sp) {
  ...
  globalThis.stepHop = function (self, pts, t, o) {
    o = o || {}
  ...
  globalThis.stepFly = function (self, sp) {
  ...
  globalThis.setPerform = function (room, on) {
    if (!room || !room.perform) return
    room.performPending = !!on
    if (!on) {
      room.performing = false
      if (room.perform.stateKey) globalThis.roomState(room, { [room.perform.stateKey]: false })
  ...
  globalThis.wirePerform = function (room) {
    const P = room && room.perform
    // 按钮从平台缝里取(见 assets/_head.js 的 ENGINE_HOST.button):
    // 设计页上是 <button>,小程序上是 WXML 的按钮,引擎两边都不认识。
    const btn = P && P.button && globalThis.ENGINE_HOST.button(P.button)
    if (!btn) throw new Error('表演态找不到按钮： #' + (P && P.button))
    btn.onTap(() => globalThis.setPerform(room, !room.performPending))
    return true
  }

  /* 房间状态机每帧调一次。helpers = { startWalk, pick }
     返回 true 表示此刻由表演态接管。 */
  globalThis.stepPerform = function (room, self, t, helpers) {
    const P = room && room.perform
    if (!P) return false
    ...
    if (room.performPending && onStage && self.mode === 'act') {
      if (!room.performing) {
        ...
        if (P.stateKey) globalThis.roomState(room, { [P.stateKey]: true })
    ...
    if (!room.performPending && onStage) {
      room.performing = false
      if (P.stateKey) globalThis.roomState(room, { [P.stateKey]: false })
      helpers.startWalk(helpers.pick())
    }

    // ⑤ 按钮文案由【状态】决定，不由点了几下决定
    const btn = P.button && globalThis.ENGINE_HOST.button(P.button)
    if (btn && P.labels) {
      const want = room.performPending ? P.labels[1] : P.labels[0]
      if (btn.getLabel() !== want) btn.setLabel(want)
    }
    return onStage
  }
  ...
  globalThis.fxFlame = function (g, x, y, opt) {
    opt = opt || {}
  ...
  globalThis.fxSmoke = function (g, x, y, opt) {
    opt = opt || {}
  ...
  globalThis.fxAura = function (g, x, y, opt) {
    opt = opt || {}
  ...
  globalThis.placeProp = function (assetId, footX, footY, opt) {
    opt = opt || {}
    const a = globalThis.ASSETS[assetId]
    if (!a) throw new Error('未知道具： ' + assetId)
    const inst = globalThis.placeAsset(assetId, footX - a.w / 2, footY - a.h, opt)
  ...
  globalThis.resolveTarget = function (spec, world) {
  ...
  // 由引擎表达。**这是明确的欠账,不是可选项。**
  globalThis.stepPursuit = function (self, spec, world, t, clamp) {
    const tgt = globalThis.resolveTarget(spec, world)
    if (!tgt) return 'idle'
  ...
  globalThis.renderRoom = function (g, room, t, actors) {
    t = t || 0
    ...
    for (const p of room.plan) {
      const e2 = ov[p[0]] || p
      const vopt = e2[3] || p[3]
      const vname = variantFor(globalThis.ASSETS[e2[0]], room, t)
      const vfin = vname ? Object.assign({}, vopt, { variant: vname }) : vopt
      try { inst.push(globalThis.placeAsset(e2[0], e2[1], e2[2], Object.assign({ room: room }, vfin))) }
