  ...
  /* ══════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════ */
  globalThis.attachRoomInteraction = function (canvas, room, opts) {
    opts = opts || {}
    ...
    function maskOf(id, x, y) {
      ...
      const inst = globalThis.placeAsset(id, x, y, {})
    ...
    canvas.__hitAt = hit
    /* 气泡状态也挂上来,理由跟上面那行一样:门禁要能问「点下去到底说没说话」。
       没有这个钩子,只能拿画布像素去猜 —— 而屋里每帧都在动,猜不准。
       有了它,「点物件有气泡」这件事就不必等真机(mini/README 里那一条)。 */
    canvas.__actorState = state

    // 指点从平台缝里接(见 assets/_head.js 的 ENGINE_HOST.onPointer)。
    // 上面那一大段 —— 逐像素遮罩、与绘制同一套的深度、追问计数 —— 与平台无关,
    // 两边共用;差别只在【事件怎么来、坐标怎么换算】:
    //   浏览器  addEventListener + getBoundingClientRect
    //   小程序  WXML 的 bindtap,由页面把坐标喂进来
    // 坐标一律用【画布像素】,换算是宿主那一侧的事。
    globalThis.ENGINE_HOST.onPointer(canvas, {
      move: function (mx, my) { state.hover = hit(mx, my); return !!state.hover },
      leave: function () { state.hover = null },
      tap: function (mx, my) {
      const h = hit(mx, my)
      if (!h) return
      const a = A[h.id]
      ...
      state.saidUntil = globalThis.ENGINE_HOST.now() + (deep ? 4200 : 2600)
  ...
  globalThis.drawInteraction = function (g, room, canvas, t) {
    const st = canvas && canvas.__interaction
    if (!st) return
    if (st.hover) {
      // 反馈用【光泽】而非描边框:框会破坏画面,光泽更像物件本身被照亮
      const a = A[st.hover.id], inst = globalThis.placeAsset(st.hover.id, st.hover.x, st.hover.y, {})
...
if (st.said && t < st.saidUntil) {
  ...
  /* ══════════════════════════════════════════════════════════════
     ...
     assetlint 里 `globalThis.lintAssets ? ... : null` 静默取到 null,
     于是「双门禁全绿」是空转出来的:S5 素材、S3 布局两步的门禁一直是空的。
     ══════════════════════════════════════════════════════════════ */

  // 素材元数据合规:尺寸 / 占地 / 分层 / 作用域 / 命名
  globalThis.lintAssets = function () {
    const bad = []
  ...
  globalThis.assetManifest = function () {
  ...
  globalThis.lintRoom = function (room) {
