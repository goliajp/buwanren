...
pub struct NajiResult {
    ...
    /* 【这一签刚才就问过了】（2026-09-06）。种子 = 谁 + 哪一天哪一小时 +
       问的那件事，所以同一小时里不写问题连摇五次是同一签 —— 那是设定。
       而屏上照旧转三秒、照旧震一下，读起来像刚算出来的一件新东西。
       返回这一位，让屏上说一句实话。 */
    #[serde(default)]
    pub again: bool,
}
...
pub struct ClientConfig {
    ...
    /// 这一格现在收不收得了钱。
    ///
    /// 【为什么要发给客户端】（2026-09-07）。繁中那一格按 TWD 标价在卖，
    /// 而全仓只有微信一个适配器、它只收 CNY —— 那一格的每一单都付不出去。
    /// 后端现在会在发起支付时说清楚（400），但那已经是**填完地址之后**了。
    /// 屏上要在他动手之前就说，跟「这个区暂时没有定价」同一个位置。
    #[serde(default)]
    pub can_pay: bool,
