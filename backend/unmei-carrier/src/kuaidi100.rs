...
fn translate_status(k100: Option<&str>) -> String {
    ...
    //   6: 退回   7: 转投   ...
    // 认不出的 code（对方将来加的、或者干脆没给 status）不再算成「在途」——
    // 那是替承运商编了一句它没说过的话。回 `unknown`：轨迹照样入库、用户看得到
    // 这一步，而 `shipment::advance_status` 对不认识的类型不动状态、只记一行 warn。
    match k100 {
        Some("1") => "picked_up".into(),
        Some("5") => "out_for_delivery".into(),
        Some("3") => "delivered".into(),
        Some("2") | Some("6") | Some("4") => "exception".into(),
        // 0 在途 / 7 转投 —— 都还在路上
        Some("0") | Some("7") => "in_transit".into(),
        _ => "unknown".into(),
    }
}
...
mod tests {
    ...
    fn translate_status_maps_correctly() {
        ...
        assert_eq!(translate_status(Some("7")), "in_transit", "转投也还在路上");
        // 这两条原先都断言 "in_transit" —— 也就是把「没给状态」和「给了个
        // 我们不认识的码」当成承运商说了「在途」。它没说。
        assert_eq!(translate_status(None), "unknown", "没给状态就是没给，不替它说在途");
        assert_eq!(translate_status(Some("99")), "unknown", "将来新增的码，别猜");
    }
