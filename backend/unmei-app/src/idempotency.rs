...
pub async fn claim(
    ...
    .bind(key)
    // 键按【用户】隔离。空值统一收敛成 ''—— 主键列不能为空,
    // 而「不带用户的调用」共用一格是可以的,跟真实用户混在一起不行。
    ...
    let row = sqlx::query(
        "SELECT response_status, response_body, request_fingerprint, method, path \
         FROM idempotency_log WHERE user_id = $2 AND key = $1",
    )
    .bind(key)
    .bind(user_id.unwrap_or(""))
    .fetch_optional(pool)
...
pub async fn complete(
    ...
    // 带上 user_id:不带的话这条 UPDATE 会落到【同名键的别人那一行】上。
    sqlx::query(
        "UPDATE idempotency_log SET response_status = $3, response_body = $4 \
         WHERE user_id = $2 AND key = $1",
    )
        .bind(key)
        .bind(user_id.unwrap_or(""))
        .bind(status)
...
/// 不碰已经落了响应的 —— 那是别人的成功结果。
pub async fn release(pool: &PgPool, key: &str, user_id: Option<&str>) -> Result<(), DomainError> {
    sqlx::query(
        "DELETE FROM idempotency_log \
         WHERE user_id = $2 AND key = $1 AND response_status = 0",
    )
        .bind(key)
        .bind(user_id.unwrap_or(""))
        .execute(pool)
