...
///
/// 最后那条 UPDATE 是有意的:今天 `payment.channel_txn_id` 的唯一写入点是
/// `apply_succeeded` 的 `COALESCE(channel_txn_id, $2)`,而 mock 渠道传进去的是
/// payment_id 本身 —— 于是这一列存的是我们自己的 id,不是渠道的流水号。
/// 对账要匹配的是渠道账单上的号,所以 fixture 直接把它设成一个渠道形态的值,
/// 让这组测试量的是【金额比对】,不被那条写入路径的问题带偏。
/// (那条路径本身是另一件事,见 README「已知欠账 · 回调匹配键」。)
async fn paid(pool: &sqlx::PgPool, amount_minor: i64) -> (String, String) {
    ...
    let pending = unmei_app::payment::start(pool, &created.order_id, &user, "wechat_jsapi", None)
        .await
        .expect("发起支付");
    unmei_app::payment::apply_succeeded(pool, &pending.payment_id, chrono::Utc::now())
        .await
        .expect("入账");

    let txn = common::uniq("4200001234");   // 渠道流水号的形状
    sqlx::query("UPDATE payment SET channel_txn_id=$1 WHERE id=$2")
        .bind(&txn).bind(&pending.payment_id)
        .execute(pool).await.expect("写渠道流水号");
