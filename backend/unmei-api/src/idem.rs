...
pub struct Guard(Option<String>);
...
impl Guard {
    ...
    pub async fn settle(self, st: &AppState, outcome: &Result<Json<Value>, ApiError>) {
        let Some(key) = self.0 else { return };
        let r = match outcome {
            Ok(Json(body)) => idempotency::complete(&st.db, &key, 200, body).await,
            Err(_) => idempotency::release(&st.db, &key).await,
        };
...
pub async fn begin(
    ...
    match idempotency::claim(&st.db, key, user_id, "POST", path, &fp).await? {
        Claim::Fresh => Ok(Begin::Proceed(Guard(Some(key.to_string())))),
