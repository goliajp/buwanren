...
由性别和年柱的阴阳定，跟好坏无关。";
    ...
    if let Some(h) = c.get("three_houses") {
        ...
        if !rows.is_empty() {
            /* 【标题要说得出这一页上有什么】（2026-09-05 · 0830 §8.3 结案）。
               这一页原先一律叫「三宫」，而列出来的是四项 ——
               三宫是胎元 / 命宫 / 身宫，旬空不是宫，它只是排盘惯例里
               附在这一页的另一件事（上面那行注释自己写的就是「三宫与旬空」，
               只有标题没跟上）。
               标题跟着行走:旬空排得出来就叫「三宫与旬空」，排不出来就还叫
               「三宫」—— 写死任何一个，都会在另一种盘上说谎。 */
            let 标题 = if 空.is_empty() { "三宫" } else { "三宫与旬空" };
            pages.push(json!({
                "key": "houses", "title": 标题,
                "rows": rows,
