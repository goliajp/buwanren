...
async fn wx_miniprogram(
    ...
    /* 没配微信凭据时，`mp_jscode2session` 回的是 `mock_openid_{code}` ——
       任意 code 都能登进去，同一个 code 永远是同一个账号。那是开发用的，
       不该在没人明说的情况下对外开着。`UNMEI_WX_MOCK=1` 才放行，
       来龙去脉见 `state.rs` 里那段。 */
    if st.wx.is_mock() && !st.allow_wx_mock {
        tracing::error!("微信凭据没配，而 UNMEI_WX_MOCK 没设 —— 拒绝这次小程序登录");
        return Err(ApiError(unmei_domain::AppError::Internal(
            "wechat mini-program login is not configured".into(),
        )));
    }
