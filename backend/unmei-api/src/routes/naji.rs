...
async fn spin(
    ...
    let rec = pick_recommend(&st.db, primary_yongshen.as_deref(), &user_region, &user_platform, seed).await?;
...
async fn check_badges(st: &AppState, user_id: &str) -> Result<(), ApiError> {
    ...
    for b in badges {
        ...
        if typ == "count" && action == "naji.spin" && count >= threshold {
            ...
            if exists.is_none() {
                sqlx::query("INSERT INTO user_badge (user_id, badge_id) VALUES ($1,$2)")
                    .bind(user_id).bind(&badge_id).execute(&st.db).await.ok();
            }
        }
    }
    Ok(())
