...
/// 填完之后屏上那一句。**说人话** —— 它是一个刚填完出生时间的人
/// 看到的第一句话，不是给同行看的批语。
///
/// 原先拼出来的是「东方 / 青绿 / 文教 / 律法 之事顺势而行。南方红黄 / 燥处宜静守。」
/// —— 半文半白、一串顿开的名词，读的人得先受过训练。
/// 现在两句:一句说该往哪儿使劲，一句说什么已经够多了。
fn build_friendly_hint(b: &BaziLite) -> String {
    let 使劲 = match b.yongshen.primary_wuxing.as_str() {
        "木" => "多往外走走，把想做的事真的动起来",
        "火" => "多往热闹的地方去，跟人待在一起",
        "土" => "把手上的事一件件做扎实，别贪多",
        "金" => "该收的收、该断的断，别拖着",
    ...
    let 够了 = b.yongshen.avoid_wuxing.first().map(|w| match w.as_str() {
        "木" => "新的计划已经够多了，先把旧的收个尾",
        "火" => "热闹和人情已经够多了，不必再往里添",
        "土" => "稳的东西已经够多了，别再往下压",
        "金" => "已经够克制了，不用再逼自己一把",
