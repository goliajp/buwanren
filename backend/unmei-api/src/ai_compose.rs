...
/// 「下午 1 点」—— 说给人听的时刻。
///
/// 原先给的是十二时辰（「未时」）。时辰是屏上不许出现的那一类词
/// （2026-08-30 定：文言只留在那一份的专业细节里），而这一栏
/// 出现在问签结果与历史列表上 —— 每天都有人读。
///
/// 分早上/上午/中午/下午/晚上/夜里，跟村主屏那句问候用同一套作息切分。
pub fn time_label(hour: u32) -> String {
    match hour {
        0..=4 => "凌晨".to_string(),
        5..=10 => format!("早上 {hour} 点"),
        11 | 12 => format!("中午 {hour} 点"),
        13..=17 => format!("下午 {} 点", hour - 12),
        18..=22 => format!("晚上 {} 点", hour - 12),
        _ => "夜里".to_string(),
    }
}
