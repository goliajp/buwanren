...
async fn update(
    ...
    a.requires_role("content")?;
    /* 先确认这条签在不在。下面五条 UPDATE 都是条件执行的，靠哪一条的影响行数
       都判不准（一条都没带的请求会「成功」而什么也没改）。

       原先 id 打错也回 200 ok:true —— 后台上看着是存好了。
       这是仓库为订单修过的那个「幽灵 ID」形状：用例层每条都守着，
       而签词这几条是在路由里直接写 SQL 的，跳过了那一层。 */
    let exists: Option<String> = sqlx::query_scalar("SELECT id FROM quote WHERE id=$1")
        .bind(&id).fetch_optional(&st.db).await?;
    if exists.is_none() {
        return Err(ApiError(AppError::NotFound(format!("quote {id}"))));
    }
...
async fn remove(
    ...
    a.requires_role("content")?;
    let n = sqlx::query("UPDATE quote SET status='archived' WHERE id=$1")
        .bind(&id).execute(&st.db).await?.rows_affected();
    if n == 0 {
        return Err(ApiError(AppError::NotFound(format!("quote {id}"))));
    }
    Ok(Json(json!({"ok": true})))
